...
class Transformer:
    ...
    def postprocess(self, result, image_shape, ori_image_shape):
        box = result['boxes']
        box[:, [0, 2]] = box[:, [0, 2]] * ori_image_shape[0] / image_shape[0]
        box[:, [1, 3]] = box[:, [1, 3]] * ori_image_shape[1] / image_shape[1]
        result['boxes'] = box
...
def expand_detection(mask, box, padding):
    ...
    h_half = (box[:, 2] - box[:, 0]) * 0.5
    w_half = (box[:, 3] - box[:, 1]) * 0.5
    y_c = (box[:, 2] + box[:, 0]) * 0.5
    x_c = (box[:, 3] + box[:, 1]) * 0.5
    ...
    box_exp[:, 0] = y_c - h_half
    box_exp[:, 2] = y_c + h_half
    box_exp[:, 1] = x_c - w_half
    box_exp[:, 3] = x_c + w_half
...
def paste_masks_in_image(mask, box, padding, image_shape):
    ...
    for m, b, im in zip(mask, box, im_mask):
        ...
        y1 = max(b[0], 0)
        x1 = max(b[1], 0)
        y2 = min(b[2], image_shape[0])
        x2 = min(b[3], image_shape[1])

        im[y1:y2, x1:x2] = m[(y1 - b[0]):(y2 - b[0]), (x1 - b[1]):(x2 - b[1])]
    return im_mask